        ...
        <div class="row">
          <card class="card-signup" header-classes="text-center" color="gray">
            ...
            <template>
              ...
              <!-- <td> -->
              <fg-input
                style="display: inline-table; min-width:250px;"
                class="no-border"
                placeholder="注册邮箱..."
            ...
            <div class="text-center">
              <n-button type="primary" round size="lg" v-bind:disabled="verifyDisabled"
              id="sendbtn" @click="handleSendForget()">验 证</n-button>
            </div>
            <td>
            <fg-input
                class="no-border"
            ...
            <td>
                <n-button style="display:inline-table;width:150px"
                type="primary" round @click="handleModify()"
                >确认修改</n-button>
              </td>

            <div class="">
                <a class="btn btn-simple btn-link"
                @click="toLogin()">去登录 >>></a>
    ...
    methods: {
        handleSendForget() {
            ...
            .then((res) => {
              ...
              this.timer = setInterval(() => {
                ...
                if (this.gap == 0) {
                  ...
                  document.getElementById('sendbtn').innerHTML = '验 证'
                  this.gap = 60
                }
              }, 1000)
            })
            .catch((err) => {
              ...
              if (err.data.error_msg === '') {
                Notification({title: '邮件发送失败', message: '邮箱未被注册', type: 'error', duration: 2000})
              } else {
        ...
        handleModify() {
            ...
            .then((res) => {
              ...
              this.$router.push('/')
              Notification({title: '修改密码成功', message: '已登录，欢迎您！', type: 'success', duration: 2000})
            })
            .catch((err) => {
              console.log(err)
              // TODO
              if (err.data.error_msg === '') {
                Notification({title: '修改密码失败', message: '验证码错误', type: 'error', duration: 2000})
              } else {
                console.log(err)
